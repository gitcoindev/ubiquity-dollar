...
import {EnumerableSet} from "../../src/dollar/libraries/EnumerableSet.sol";
...
contract EnumerableSetTest is Test {
    ...
    EnumerableSet.Bytes32Set mbs;
    EnumerableSet.AddressSet mas;
    EnumerableSet.UintSet mus;

    function testEnumerableSetBytes32SetOperations() public {
        // empty
        assertEq(mbs.length(), 0);
        assertEq(mbs.toArray().length, 0);
        ...
        assertEq(mbs.length(), 2);
        bytes32[] memory values = mbs.toArray();
        assertEq("a", values[0]);
        assertEq("b", values[1]);
    ...
    function testEnumerableSetAddressSetOperations() public {
        // empty
        assertEq(mas.length(), 0);
        assertEq(mas.toArray().length, 0);
        ...
        assertEq(mas.length(), 2);
        address[] memory values = mas.toArray();
    ...
    function testEnumerableSetUintSetOperations() public {
        // empty
        assertEq(mus.length(), 0);
        assertEq(mus.toArray().length, 0);
        ...
        assertEq(mus.length(), 2);
        uint[] memory values = mus.toArray();
        assertEq(1, values[0]);
        assertEq(2, values[1]);
